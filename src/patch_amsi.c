...
const char MIMIKATZ_SAMPLE[] = "C:\\Users\\whokilleddb\\Samples\\mim\\x64\\mimikatz.exe";
...
int scan_buffer(const char *path){
    ...
    if (amsi == NULL){
        fprintf(stderr, "[!] Failed to load amsi.dll(%d)\n", GetLastError());
        return -1;
    }
    ...
    if (file == INVALID_HANDLE_VALUE) {
        fprintf(stderr, "[!] Failed to get a valid handle to %s(%d)\n", path, GetLastError());
        return -2;
    }
    ...
    if (map == NULL){
        fprintf(stderr, "[!] Failed to create file mapping(%d)\n", GetLastError());
        return -4;
    }
